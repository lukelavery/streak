...
final streakControllerProvider =
    StateNotifierProvider.autoDispose<StreakController, AsyncValue<Map<String, List<Streak>>>>(
        (ref) => StreakController(ref.read));

class StreakController extends StateNotifier<AsyncValue<Map<String, List<Streak>>>> {
  ...
  Future<void> addStreak({required String habitId, required DateTime dateTime}) async {
    await _read(streakServiceProvider).addStreak(habitId: habitId, dateTime: dateTime);
  }
}